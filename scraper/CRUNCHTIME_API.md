...
**⚠️ CONFIRMED RISK (2026-07-11) — no locking around `deposits_pending.json` / `enter_ct_deposits.py` lets two processes double-post the same deposit.** Incident: a manager-queued backup entry (business_date 2026-07-10, $634.51) sat in `pending[]` since `2026-07-11T16:37:52Z`. At `2026-07-11T16:51:49Z` the scheduled hourly workflow run started (`gh run list` confirms it); **in the same minute**, a manual/local invocation of `enter_ct_deposits.py` also started (for an unrelated diagnostic pass). Both processes fetched `origin/main` before either had pushed, so both saw the same entry still sitting in `pending[]`, and **both independently posted a $634.51 deposit** for salesId 6287459 via `/save`+`/submit`. Result: `POST /resource/salestransactions/bankdeposits` for that salesId showed **two** rows (`depositId 31238817` and `31238822`, both $634.51 — total $1269.02 instead of $634.51). `registerSales/summary.totBankDeposits` for that date jumped from `$0.00` straight to `$1269.02` and `over_short` flipped from `-634.51` to `+634.51`. Re-opening the salesId's edit URL afterward also surfaced a **"Sales Transaction Alert: previous sales transaction not submitted — Continue / Delete / Cancel"** modal, consistent with two near-simultaneous Playwright/session flows touching the same transaction.

**Do not click Continue/Delete/Cancel on that modal via automation, and do not attempt to zero/delete a bankdeposits row automatically** — the read endpoint returns only `depositId`+`amount`+`memo` (no timestamp/user/audit field anywhere in this API surface), so there is no reliable way to tell which row is the duplicate. This needs a human in the CT UI (Sales Transactions → edit salesId → Bank Deposits grid) to resolve.

**The real fix is a lock, not a retry.** Before `enter_ct_deposits.py` processes an entry, it should claim it (e.g. write a `claimed_by`/`claimed_at` field and re-read to confirm no other process claimed it first, or take an OS/repo-level lock) so the hourly cron and any manual/local run can't both act on the same `pending[]` entry. Until that lock exists, **never run `enter_ct_deposits.py` manually without first checking `gh run list --workflow=refresh_ct_deposits.yml` for an in-flight run in the last ~5 minutes**, and treat every backup-queued write as needing a same-day CT UI duplicate spot-check (compare `ct_sales_summary_history.json`'s `deposit` value against the queued `amount` — if `deposit` is a near-exact multiple of `amount`, suspect a duplicate).
...
**WRITE ENDPOINT — CONFIRMED LIVE 2026-08-07:**

```
POST /resource/inventorysetup/products/save
Body: [<full product row object, only productSequence changed>]
Response: {"success": true, "contentMap": {}}
```

The body is the **entire row** as returned by `/resource/inventorysetup/products` (same shape shown above), not a partial `{id, sequence}` patch — send the full row back with just `productSequence` edited (drop the ExtJS-only `index` key the grid store attaches; everything else must round-trip unchanged). Verified end-to-end live against KY-2065 (test product `P00053` in `COGS: Freestyle`, storageId 39539, sequence 1↔2 swapped and reverted, both directions returned `{"success":true}` and re-read confirmed the new order). Also discovered: clicking the "Sequence" toggle button in the UI fires `POST /resource/inventorysetup/resequence` `{"extraCriteriaMap":{"storageId":<id>}}` — this appears to be a bulk "compact to consecutive integers" action (renumbers a storage location's items to clean 1,2,3… based on current order), separate from the single-row `/save` call; harmless when items are already sequential, not yet tested on a location with real gaps.

**Order guide / Create Vendor Order (truck order) — CONFIRMED same sequence concept, 2026-08-07.** Drove the full wizard (Purchasing → Create Vendor Order → pick vendor "Performance Fdsvce" → Continue) live. `POST /resource/purchasing/vendororder/create` returns `vendorOrderId: null` / `hdrVendorOrderPk: null` — Continue only stages an in-session draft, nothing is persisted to the DB until an explicit Submit (confirmed: "Recent Vendor Orders" total stayed at 87 before/after, no phantom order created — safe to drive this wizard read-only). The item grid, `POST /resource/purchasing/vendororder/edit/list/`, defaults `simpleFilterMap.sort` to **`"inv_seq"`** (inventory sequence) and each row carries both a `sequence` and a `productSequence` field — same concept as Inventory Setup's Sequence tab, just null on rows that haven't been assigned one yet (matches the gap pattern seen in Inventory Setup for items never explicitly sequenced). **Conclusion: reordering a GL/storage location's Sequence in Inventory Setup is expected to reorder that same section on the vendor-order/truck-order screen too** — one edit, both surfaces. Not yet verified with an actual bulk reorder (only single-item edits were tested) — sanity-check the order guide screen the first time a full section gets reordered for real.

**Bulk-reorder script:** `scraper/ct_reorder_gl_sequence.py` — pure `requests`, no browser at runtime. `--list "<storage name>"` to read current order (read-only), `--storage "<name>" --order P00004,P00001,...` (or `--order-file`) to reorder, `--dry-run` to preview without writing. Loops one `/save` POST per changed row (proven-safe pattern; batch-array `/save` was not verified). See the script's module docstring for full usage and the write-safety notes (never touches active flags, never adds/removes items, never creates/submits an order).
...
- **The Performance Metrics *widget* (DOM scrape) renders whatever week CrunchTime defaults to — on a MONDAY run that is the UPCOMING Mon–Sun week, not the week containing yesterday** (confirmed 2026-08-24). Grid headers read `Monday 08/24/2026 … Sunday 08/30/2026` while yesterday was Sunday 08/23. Every per-day `Actual` cell is blank in that state, so the only values on each row are Week-to-Date / Period-to-Date. `scraper/main.py::_extract_from_page_text` used to fall back to `values[0]` and silently wrote the **PTD total in as yesterday's net sales**: 2026-08-16 → `$74,629` and 2026-08-23 → `$108,118`, against a real day of ~$4.4K (~23× too high). Non-Monday runs were unaffected, so this went unnoticed for at least two weeks.
  - **Fixed 2026-08-24** — `_extract_from_page_text` now parses the grid's own `Weekday MM/DD/YYYY` headers (anchored to the "Performance Metrics" heading, since other widgets on the page carry their own dates), matches yesterday against them, and **returns `{}` when yesterday is not in the displayed week** rather than reading a total as a daily value. The per-row fallback to `values[0]` is gone; a missing day cell now yields `""` → `None`.
  - **Consequence is by design:** on Mondays the DOM scrape yields no CT day metrics and `wire_dashboard.py`'s Par Brink fallback owns Net Sales (verified: dashboard showed the correct `$4,372` throughout). LY / Forecast / Sales-per-Guest have **no Par Brink fallback** and now render blank on Mondays instead of showing the wrong week's numbers.
  - **CLOSED 2026-09-07 — date-parameterized backfill shipped as `scraper/ct_daily_payroll.py`.**
    - First: `/resource/dashboard/performance/metrics` is **NOT date-parameterizable**. Probed 2026-09-07 with `startDate`/`endDate` (in and out of `extraCriteriaMap`), `businessDate`, and `weekEndingDate` — all four returned byte-identical current-week payloads (`2=None … 1=None, WTD=None, PTD=27253.31`). Don't retry this; the widget endpoint always serves CT's default week.
    - `/resource/dailypayrollcontrol/summary` **is** date-parameterized and works. Gotcha: rows come back under **`contentMap.gridList`**, not `data` or `rows` — a naive `j.get("data") or j.get("rows")` parse yields 0 rows and looks like an empty response.
    - **Reconciliation pass (the one the old note demanded), 2026-09-07** — every field verified against the known-good 2026-09-05 record in `latest.json` *and* the Par Brink Sales Summary PDF:

      | Field | API | Cross-check | Match |
      |---|---|---|---|
      | `actualSales` 09/05 | 5521.06 | Par Brink `net_sales` 5521.06 / latest.json 5521 | ✅ exact |
      | `actualSales` 09/06 | 4818.82 | Par Brink `net_sales` 4818.82 | ✅ exact |
      | `forecastedSales` 09/05 | 5398 | latest.json `forecast` 5398 | ✅ exact |
      | Σ `actualSales` 08/31–09/05 | 25921.22 | latest.json `net_week` 25921 | ✅ exact |
      | Σ `forecastedSales` 08/31–09/05 | 27838 | latest.json `forecast_week` 27838 | ✅ exact |

    - **Sales family only — payroll is still excluded.** The 2026-07-13 warning below stands and was re-confirmed: 09/06 `actualPayroll` = 1636.68 (33.96%) against Par Brink `labor_cost` 977.35 (20.28%). The last day of the week appears to carry a weekly salaried allocation. Labor stays on its existing chain (CT API → Par Brink hourly PDF in `wire_dashboard.py`).
    - **Still blank on Mondays:** LY and Sales-per-Guest. This endpoint carries neither. LY is obtainable from `registerSales/summary` against last year's range (see § 1.4 and its Sunday-offset caveat) — next build if Bobby wants those cells filled Mondays.
    - Wired into `scraper/main.py::main()` — when the DOM scrape raises, it tries `ct_daily_payroll.fetch_day(yesterday)` **before** falling through to the cached-`latest.json` branch. Before this, every Monday dropped to cache and froze the dashboard on Saturday's numbers until Tuesday.
- **`/resource/dashboard/performance/metrics`'s per-day (calcId) "Actual" metric family can be null for an ENTIRE new week, including already-completed days** (confirmed 2026-07-13). When a new Sun-Sat week starts, `Actual Net Sales`/`Actual Labor`/`Labor % of Net Sales`/`Actual Hours` came back `null` for every calcId (1-7) — including calcId=1 (Sunday, already a completed day) — while `PTD` (period-to-date) still had real values, and the purely-forward `Scheduled Hours` metric was fully populated for all 7 days. This is CT posting latency for the new week's day-level grid, **not a calcId/day-of-week mapping bug** — verified by checking that Scheduled Hours (same grid, same calcIds) populates fine while every Actual-family row is null across the board. Symptom in `scrape_labor_ct.py`: "got 37 metric rows" but Labor%/Labor$/ActualHrs all extract as `None`. **wire_dashboard.py's existing Par Brink PDF fallback correctly covers this** — no dashboard-visible break.
  - **Tried and REJECTED as fallback sources** (2026-07-13): `/resource/dailypayrollcontrol/summary` (date-keyed `actualPayroll`/`actualPayrollPercent`) and `/resource/nc/labor/productivity/summary` (`summary.totalLaborHours.actual` × `summary.averageHourlyWage`) BOTH return real, non-null data for the same date — but neither reconciles with the trusted `performance/metrics` "Actual Labor" number on a known-good day (07/11: performance/metrics=$948.19/17.86%/86.81hrs vs dailypayrollcontrol=$1028.19/19.37% [+8%] vs labor/productivity-derived=$1151.14/21.69%/83.33hrs [+21%/-4%]). They use a different labor-cost/hours methodology (likely burden or employee-scope differences) and are **NOT safe drop-in substitutes** — do not wire either in as a numeric fallback without a real reconciliation pass against several known-good days first. `scrape_labor_ct.py` deliberately returns `null` and lets the existing Par Brink fallback handle it rather than risk shipping a silently-wrong number.
