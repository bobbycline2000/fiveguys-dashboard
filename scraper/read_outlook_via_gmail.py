...
def build_shift_huddle_plan(today: date, categorized: dict[str, list[dict]] | None = None) -> str:
    ...
    rotating_ideas = {
        0: ("Monday -- Changeover Day: Bread, Potatoes, Cash, and a Clean CrunchTime Board",
            f"Biggest supply-chain week we have had all year, so open with it. {shop_phrase}. "
            f"{month_ytd_phrase} Now the Monday reset, four items. "
            "**(1) BREAD CHANGEOVER -- Jeff Reiss directive.** Today's BOSS delivery has to carry us "
            "**24 hours past the Performance truck -- through Tuesday afternoon.** After today's delivery "
            "lands, **zero out every remaining BOSS delivery.** Going forward bread comes on the "
            "**Performance truck only, twice a week, 10 bags to a case** -- so bread goes on the next "
            "Performance order or we run out. Next-generation bread is supposed to be in the store this "
            "week. Anything unclear, Jeff direct: **859.481.2675.** "
            "**(2) NEW POTATOES START THIS WEEK** -- the Idaho Norkotah crop transition begins Sept 1. "
            "Different moisture and sugar than what we have been cutting: **watch fry color and cook time "
            "all week**, taste-test the first batch of every shift, and speak up if they are coming out "
            "dark or limp. Do not change the portion -- change the timer. "
            f"**(3) CASH** -- {deposit_phrase}. "
            "**(4) CLEAR THE CRUNCHTIME BOARD** this morning -- post labor reviews and receive vendor "
            "orders before noon. **L-Rev stacked three deep again last week**; that is the pattern that "
            "keeps getting us flagged. Standing: **Bread Policy signatures are past due** -- every black "
            "shirt signs today, one email with all of them to Brad. CO2 under 25% -> order and confirm the "
            "50 lb backup. All in-store tech goes to the **Universal Service Desk, 888-411-0083 / "
            "FiveGuysHelp.Zendesk.com.** Scams still live -- no email, login, cash, or shipping address "
            "over the phone, ever."),
        1: ("Tuesday -- The Red Flag Report: Ticket Times and Drinks, Fries, Shakes",
            ...
            f"is 30 cents up. {kpi_phrase}. {shift_phrase}. Bonus money is sitting on the table -- these "
            "two behaviors are how the crew picks it up."),
        2: ("Wednesday -- COGS Check: New Potatoes, Same Standard",
            f"{cogs_phrase}. We are winning the cost side right now -- do not give it back on the "
            "changeover. **New Idaho Norkotah crop is in the pipeline**, so fries are the watch item all "
            "week: **check color and cook time every batch, taste one, adjust the timer -- never the "
            "scoop.** Portion the fries level, measure toppings and condiments, cook to the board and not "
            "to a guess so we are not dumping meat at the end of a rush, and log every waste cup as it "
            "happens. Check the variance list on the dashboard for today's worst line and re-drill that "
            "station's build with everyone who touches it. **Wednesday close: the Milkshake Pump Cleaning "
            "list gets done and signed tonight, by the closer, before they leave** -- that is the day we "
            f"told ComplianceMate we do it, so it has to be true. {labor_phrase} -- food and labor "
            "together are what pay the bonus, and right now both halves are winning. Protect it."),
        3: ("Thursday -- Steritech Standard: We Earned a 99, Now Hold It",
            "**We finished at 99 on the Food Safety Assessment** -- the FSA challenge went our way and it "
            "went all the way to George Estep. That number is not a trophy, it is the new floor. Check "
            "today's ComplianceMate status before doors -- if any section is red or incomplete from last "
            "night, it gets closed before the shift starts, not backfilled later. **PROBE PROTOCOL WHILE "
            "WE ARE DOWN: every temp round gets taken and WRITTEN DOWN ON TIME, on paper, by the hour it "
            "is due -- never reconstructed at 10 PM.** When a probe comes back online, enter them. If the "
            "app will not pair: confirm ComplianceMate is on **6.2.14**, restart the tablet, unpair every "
            "probe in Bluetooth settings, Bluetooth off then on, Location ON, Wi-Fi on, close and reopen "
            "the app, then three lines -> Probes -> Search -> Activate Probe. Still dead, reply on the open "
            "ticket and ask for the QR code. The rest of the standard does not move: **manager walk "
            "completed and SIGNED before doors** -- hand sinks for handwashing ONLY, DayMark discard dates "
            "current, potato-cutter teeth and slicer blades chip-free, **lids on mushrooms and onions**, "
            "wet dishes stacked, sanitizer buckets changed on schedule, veggie wash tested and logged, "
            "expired product out of the walk-in, hood degreased, mop sink clean, line trash cans with no "
            "liquid or debris. **The closing manager does not leave until the Closing Checklist and every "
            "time-and-temp are complete and signed -- on time, never backfilled.** We missed the Closing "
            "Checklist twice in the last five recorded days; that stops this week."),
        4: ("Friday -- Shop Review: Saturday Is Where We Lose the Points",
            f"Pull the board and read it out loud. {shop_phrase}. {month_ytd_phrase} {mgr_phrase} Here is "
            "the pattern nobody wants to say out loud: **both of our 95s this month landed on a "
            "Saturday**, and both lost the points on **service and guest recovery -- not the food.** So "
            "the weekend crew is the crew that decides our score. The action plan stays one behavior: "
            "**MOD at the table within 60 seconds of any unhappy guest, five proactive table-touches a "
            "shift, logged on the clipboard** -- anybody on the crew can flag it, nobody waits to be "
            f"asked. {shift_phrase}. And Brad's two: **tickets under 6 minutes** and **drinks, fries, "
            "shakes offered out loud to every guest.** The rest lives where it always lives: greet at the "
            "door and the register, every bag checked against the ticket, open signs on before doors, no "
            "phones on the register. Set the weekend crew up to win the next one."),
        5: ("Saturday -- Biggest Volume Day: A Shop Day With Full Prep",
            "Saturday is our biggest day and it is **the day our shops come in lowest** -- our last two "
            "95s were both Saturdays. Treat every guest like the shopper, because on a Saturday they "
            "might be. Full prep, par up bread and potatoes early -- **bread is on a two-deliveries-a-week "
            "cycle now, so what is in the freezer is what we have** -- and everybody knows their backup "
            "station **before** the rush, not during it. Standing rule, no exceptions: **food that touches "
            "the floor is thrown away and remade -- bagged or not** -- and **you never correct a crew "
            f"member in front of a guest.** Run the day: {labor_phrase} -- hold it the way we earned it, "
            "**check the number hourly**, trim openers and mids on a real lull, **never the close**, "
            "**30-minute breaks actually taken**, **employee meals rung in at the time of the break.** "
            "Sell into the traffic -- **drinks, fries, and shakes suggested out loud to every guest**, "
            "Large soda 30 cents up. Keep tickets **under 6 minutes**, fastest fry person at peak, watch "
            "the new-crop fry color, **check every bag against the ticket before it is sealed.** "
        ...
        6: ("Sunday -- Finish the Week: Order Ahead, Close Signed, Reset for Monday",
            ...
            "backfilled Monday morning.** Anything genuinely unfinished gets written down and handed to "
            "the opener. **ORDER AHEAD BEFORE YOU LEAVE:** bread now rides the **Performance truck only, "
            "twice a week, 10 bags a case** -- put it on the order or we run out midweek, and on a **Labor "
            "Day week the Performance truck slides a day (Monday's load moves to Tuesday, Tuesday's to "
            "Wednesday), so order Sunday for Tuesday.** Then close the week the rest of the way: re-par "
            "what the weekend drained (bread, potatoes, condiments, cups), DayMark labels current, waste "
            f"sheet honest, {deposit_phrase}, **CT tasks closed the day they land** -- on period-close "
            "Sunday that means labor posted and inventory finalized. Recap the week for the team so they "
            f"hear the whole picture: {cogs_phrase}. {labor_phrase}. {shop_phrase}. Hand the openers a "
            "Monday-ready store."),
    }
    biweekly_themes = [
        ("Even Weeks -- Finish the Shift: Signed Closings, Clean Cash, and a Clean Changeover",
         "This week is about **finishing what we start** -- and we are finishing it in the middle of the "
         "biggest product change of the year. **(1) THE CHANGEOVER.** Bread moves to the **Performance "
         "truck only, twice a week, 10 bags a case** -- **zero out the remaining BOSS deliveries after "
         "Monday**, and put bread on every Performance order from here forward. The **Idaho Norkotah "
         "potato crop transitions Sept 1** -- new moisture and sugar means **watch fry color and cook "
         "time all week, taste the first batch every shift, adjust the timer and never the portion.** "
         "**(2) THE CLOSE.** The standard: **the closing manager does not leave until the Closing "
         "Checklist and every time-and-temp are complete and signed -- on time, never backfilled.** We "
         "missed the Closing Checklist twice in the last five recorded days. Pre Open, Weekly Store "
         "Inspection and Delivery Check each get an owner **by name**, and the **Milkshake Pump Cleaning "
         f"list is signed Wednesday night.** **(3) THE CASH.** {deposit_phrase}. Count the safe and both "
         "drawers at open ($700 / $200 / $200), count at close, drop to the safe, prep the deposit, log "
         "it on the Safe & Drawer page, and **enter it in CrunchTime -- every night.** **(4) THE "
         "TRAINING + THE PAPERWORK.** Check Five Guys University this week -- everyone under 100% gets a "
         "completion date, anyone past an onboarding deadline comes off the schedule per the standard. "
         "**Bread Policy signatures are past due -- every black shirt signs, one email to Brad.** "
         "**(5) THE INSPECTION STANDARD underneath all of it:** we finished the FSA at **99** -- that is "
         "the floor now. Manager walk completed and **SIGNED before doors** -- hand sinks for handwashing "
         "only, DayMark dates current, cutter teeth and slicer blades chip-free, **lids on mushrooms and "
         "onions**, wet dishes stacked, sanitizer buckets changed, veggie wash tested and logged, expired "
         "product out of the walk-in, hood degreased, line trash with no liquid or debris, restroom "
         "checked hourly on a signed sheet. **While the probes are down, every temp is written by hand ON "
         "TIME and entered when they are back** -- never reconstructed. Sensors quiet? Check power and "
         "internet, then **Universal Service Desk 888-411-0083 / FiveGuysHelp.Zendesk.com.** Scams still "
         "live -- nothing over the phone, ever."),
        ("Odd Weeks -- The Red Flag Sprint: Ticket Times, Drinks-Fries-Shakes, and Guest Recovery",
         "Brad sent the Red Flag Report and named the two things holding our KPIs back, so this is the "
         "whole sprint. **(1) TICKET TIMES.** *Head on a swivel and eliminate bottlenecks.* **Under 6 "
         "minutes register to window** -- over 8 is -5, over 10 is -20. The **fry station is the "
         "bottleneck**: pre-drop into the rush, fastest fry person at peak, expo watching for the one "
         "order stalling the line, and when the shift changes somebody still owns the door. **(2) DRINKS, "
         "FRIES, AND SHAKES -- SUGGESTED TO EVERY GUEST, OUT LOUD.** That is the Halo Effect and it is "
         "worth real bonus money: greet at the door AND the register, eye contact, order called back at "
         "handoff, fries-up, a real thank-you and goodbye, **every bag checked against the ticket**, "
         f"Large soda 30 cents up, open signs on before doors, **no phones on the register.** {kpi_phrase}. "
         f"{shift_phrase}. **(3) GUEST RECOVERY IS A SCORED SKILL.** {shop_phrase}. **Both of this "
         "month's 95s were Saturdays and both lost the points on service, not food.** The plan is live: "
         "**MOD at the table within 60 seconds of any unhappy guest, five proactive table-touches per "
         "shift, logged on the BOH clipboard**, any crew member can flag it. **Food that touches the "
         "floor is remade, every time**, and **you never correct a crew member in front of a guest.** "
         f"**(4) THE MONEY IS BEING WATCHED -- DEFEND IT.** {cogs_phrase}. {labor_phrase}. Portion fries "
         "level and **watch the new-crop color and cook time**, measure toppings, cook to the board, "
         "spot-check patty weight and cheese count, check labor **hourly**, cut on a real lull and "
         "**never the close**, 30-minute breaks actually taken, **employee meals rung in.** Standing "
         "items that keep biting us: **CO2 under 25% -> order and confirm the 50 lb backup**, **bread on "
         "every Performance order -- it only comes twice a week now**, **close CT tasks the day they "
         f"land**, {deposit_phrase}, **Closing Checklist signed before the closing manager leaves.** "
         "Scams still live -- nothing over the phone, ever."),
    ]
