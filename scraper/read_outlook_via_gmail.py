...
def build_shift_huddle_plan(today: date, categorized: dict[str, list[dict]] | None = None) -> str:
    ...
    rotating_ideas = {
        0: ("Monday — Phone Scam Lockdown + Clear the Task Stack",
            "Start the week with the security brief — Brad sent a **SCAM ALERT** (7/17) and it is "
            "still live: callers are posing as **UBER with 'connectivity issues'** and asking for the "
            "store **email and login**. Locations got hit overnight. **NO ONE gives email or login "
            "info over the phone, ever** — nobody from ownership or corporate will ever call asking. "
            "Print Brad's email and post it by the phone; every cashier and every MOD hears this "
            "today. Second Monday job — it's period close and CT tasks are stacked four deep "
            "(L-Rev 7/16, 7/17, 7/18, 7/19, plus VO-Rec BOSS and 'Post Period — Labor Not Posted'). "
            "Clear them this morning; Brad has called the stacking out repeatedly. Also today: the "
            "internet is intermittent (delivery got shut off indefinitely because drivers were "
            "arriving for orders we never received) — confirm the store shows delivery OFF on "
            "fiveguys.com/OLO, and get a hub ticket filed on the internet. CO2 check stays daily: "
            "under 25%, order now; confirm the 50 lb backup tank. Open with the number — the week "
            "ending 7/19 closed **$31,328, +13.0% over last year**."),
        1: ("Tuesday — Crystal's KPI Three, and We're Losing the Biggest One",
            "Crystal's KPI Ranking is 60% of our whole score — **Secret Shop scores, Customer "
            "complaints (CSPs), and Ticket times** — and right now the shop leg is where we're "
            "bleeding. July sits at **57.3% MTD** and Brad put us **at the bottom of the district**. "
            "Don't soften it at huddle: tell the team where we rank, then tell them exactly how we "
            "climb. (1) **SHOPS** — the Halo Effect is the scored behavior: warm greet at the door "
            "AND the register, order called back at handoff, fries-up, a real thank-you and goodbye. "
            "(2) **COMPLAINTS** — catch an unhappy guest EARLY, at the table, before it becomes a "
            "CSP; a warm recovery kills a bad review. (3) **TICKET TIMES** — under 6 minutes "
            "register-to-window, every ticket. Spot-check the register unannounced (fry, drink, AND "
            "shake — Crystal's exact words). Grill culture the standard way: 2 for guys / 1 for "
            "girls. We're **#1 on Incidence Sales** company-wide — Blackberry shake (LTO thru Aug "
            "16), drink, and fries on every ticket is how we keep it. Large soda is 30¢ more."),
        2: ("Wednesday — Cheeseburger Waste: We're $2,819 Over on One Item",
            "Money leak, named and measured. The last closed food-cost week (7/6–7/12) ran "
            "**$7,001 over theoretical**, and one item is most of it: **Cheeseburger, $2,819 over** "
            "— actual $3,647 against a theoretical $828. That is not a rounding error, that is patty "
            "count and grill waste. Today's play: spot-check the **3.5 oz patty roll** at the grill "
            "— weigh a few, correct anybody rolling heavy; cook to the board, not to a guess, so we "
            "aren't dumping cooked meat at the end of a rush; and every single waste item gets "
            "**logged** — an unlogged throw-away is what turns into a variance nobody can explain. "
            "Same discipline on the line: fries scooped to spec (not heaped), toppings to spec, lids "
            "on. Sales are strong (+13.0% over last year for the week ending 7/19) — a strong top "
            "line only pays us if the food cost holds underneath it. While the grill is tight, hold "
            "ticket times under 6 minutes: the fry team is still the named bottleneck (over 8 min = "
            "-5, over 10 min = -20), so station your strongest fry person at peak and pre-drop into "
            "the rush. Meat on the grill BEFORE the order when 3+ are in line."),
        3: ("Thursday — Produce Safety: Veggie Wash Tested, Logged, Used on Everything",
            "Food safety is the store-wide #1 right now — Brad, Crystal and Corporate all pushed it "
            "this week off the **Cyclosporiasis** outbreak (CDC/FDA traced multistate Cyclospora to "
            "shredded iceberg lettuce at another brand — **not** Five Guys product, but the drill is "
            "on us anyway, and cashiers should know that answer if a guest asks). Today, every "
            "shift: **test the veggie wash and log it**, wash and sanitize ALL produce, sanitize "
            "utensils, surfaces, dining room and restrooms, wipe the Freestyle, hand-wash and change "
            "gloves between tasks, and **change sanitizer buckets** on schedule. Then the standard "
            "walk before doors open and sign off: hand sinks for handwashing ONLY, DayMark discard "
            "dates correct, potato-cutter teeth and slicer blades chip-free (1704 CRITICAL), lids on "
            "mushrooms and onions, wet dishes stacked. Temps are a named accountability item this "
            "week — Brad forwarded a 'Short temp' report after 7/16 — so every time-and-temp gets "
            "logged **on time**, not backfilled. Restroom is still our #2 systemic shop miss (two "
            "shops at -20 for dirty floor + dirty sink): named owner, hourly check, signed sheet, a "
            "real walk-in during the dinner rush. July heat is real — log refrigeration temps."),
        4: ("Friday — Shop Review: Off the Bottom. Brad Asked for #1.",
            ...
            "Effect!\"* Where we actually are: **July 57.3% MTD**, store YTD 83.2% over 40 shops. "
            "The three July shops — **7/12 Lunch 87%** (Francisco Llorente Mejias, Madison Cureton, "
            "Vicki Lucey), **7/11 Dinner 0%** (Jeremiah Matthews, Madison Cureton, Richard Gibbs), "
            "**7/03 Lunch 85%** (Robert Cline, Francisco Llorente Mejias, Jeremiah Matthews, Lidy "
            "Henry, Maylin Hernandez Rodriguez, Vicki Lucey). Pull the **7/11 zero** first and read "
            "the actual report before coaching anyone — a 0 is either a total-miss visit or a "
            "reporting anomaly, and the crew deserves the facts, not a verdict. Then coach the real "
            "gaps by name off the 87% and the 85%. Remind them what this store is capable of: June "
            "was three straight 100% shops (6/21, 6/24, 6/25) and closed 95.0% — the standard is not "
            "theoretical, we hit it eight weeks ago. The points live in the **HALO EFFECT** (warm "
            "greet at door and register, order called back, fries-up, real thank-you and goodbye), "
            "ticket times under 6 minutes, and the **MILKSHAKE offer** — still our most-dropped "
            "upsell. Today every cashier offers the Blackberry shake out loud on every order (LTO "
            "thru Aug 16). Set the weekend crew up."),
        5: ("Saturday — Weekend Volume: Sell Hard, and Defend the Labor Win",
            ...
            "person, pre-drop aggressively, hold ticket times UNDER 6 MINUTES. **Labor flipped from "
            "problem to win this week — protect it.** We ran **23.0% for the week** and Sunday 7/19 "
            "came in at **18.83%**, both under Brad's 25% ceiling; that is exactly what he's been "
            "asking for since 7/9. Hold it the same way we earned it — check the number hourly, trim "
            "openers/mids on a real lull, never the close, 30-min breaks taken, employee meals rung "
            "in. Steritech-clean all day: lids on mushrooms and onions, DayMark dates current, "
            "cutter + slicer chip-free, restroom checked hourly with a signed sheet. Cash discipline "
            "too — last Friday's deposit went unentered and showed as a **−$433.90 short** until it "
            "was found; count it, enter it, don't leave it for Monday."),
        6: ("Sunday — The Closing Checklist Gets Signed. Four Misses Is Enough.",
            "Straight talk at huddle: **the Closing Checklist was missed four times in five days** — "
            "7/15 (Anthony Pirtle, plus the Milkshake Pump Cleaning check), 7/16 (Madison Cureton, "
            "plus the 5PM Time & Temp — Brad emailed a 'Short temp' notice about that one), 7/17 "
            "(Christopher Conder), and 7/19 (Madison Cureton, plus the PM Pre-Shift). That's not one "
            "person having a bad night, that's the close coming apart. Fix it tonight and make it "
            "the standard: **the closing manager does not leave until the Closing Checklist and "
            "every time-and-temp are complete and signed** — on time, not backfilled the next "
            "morning. If something genuinely can't be completed, it gets written down and handed to "
            "the opener, not skipped silently. Then the rest of the close: re-par what the weekend "
            "drained (bread, potatoes, condiments, cups), DayMark labels current, drawer counts "
            "squared and the deposit ENTERED, waste sheet honest, and **CT tasks closed — L-Rev has "
            "stacked four deep into this Monday** and Brad has called it out repeatedly. Recap the "
            "week for the team: sales are strong (**+13.0% over last year**, WE 7/19) and labor "
            "landed at **23%**, under the ceiling — the floor work is winning. Shops are where we're "
            "behind, so full floor discipline holds right through the last hour: Halo greet at door "
            "and register, tickets under 6 minutes, meat on the grill before the order, shake "
            "offered every ticket, restrooms checked hourly with a signed sheet. Hand the openers a "
            "Monday-ready store."),
    }
    biweekly_themes = [
        ("Even Weeks — Get Our Shoppers Off the Bottom: Brad Asked for #1",
         ...
         "Celebrate our Halo Effect!\"* We're at **57.3% for July** against a store YTD of 83.2% — "
         "and shops are the biggest leg of Crystal's KPI Ranking, where **60% of our score is Secret "
         "Shops + Complaints + Ticket times.** The climb is three moves, run them every shift: "
         "(1) **HALO EFFECT** — this is the scored behavior, not a slogan: warm greet at the door "
         "AND the register, order called back at handoff, fries-up, a real thank-you and goodbye. "
         "Every guest leaves with a 'Wow.' (2) **TICKET TIMES UNDER 6 MINUTES** — the fry team is "
         "the named bottleneck (over 8 min = -5, over 10 min = -20); station your strongest fry "
         "person at peak and pre-drop into the rush. (3) **THE OFFER, OUT LOUD** — milkshake, drink, "
         "fries on every ticket; the shake is still our most-dropped upsell and we are **#1 in "
         "Incidence Sales company-wide** — we hold that by asking every time (Blackberry LTO thru "
         "Aug 16, Large soda 30¢ up). Coach unhappy guests EARLY, at the table, before they become "
         "a CSP. Two things to keep in front of the team while we climb: this store went three "
         "straight 100% shops in June (6/21, 6/24, 6/25) and closed the month at 95.0% — the "
         "standard is proven, not aspirational; and the bench got stronger this month with Rusul "
         "(100), Anthony (99) and Madelynn (98) all certified. Pull the **7/11 zero** and read the "
         "full report before coaching off it."),
        ("Odd Weeks — Hold the Labor Win, Kill the Food-Cost Leak",
         "Two numbers, opposite directions — celebrate one, fix the other. (1) **LABOR — we won it, "
         "now defend it.** Brad has pushed 'under 25%' since 7/9, and the week ending 7/19 landed at "
         "**23.0%**, with Sunday at **18.83%**. That is the ask, met. Tell the team so — they earned "
         "it — then hold it exactly the way it was earned: check the number hourly, trim openers and "
         "mids on a real lull, **never the close**, 30-min breaks actually taken, employee meals "
         "rung in. Sales are up **+13.0% over last year**, so hours are affordable where the volume "
         "is; the discipline is in the slow patches. (2) **FOOD COST — that's where the money is "
         "leaking now.** The last closed week ran **$7,001 over theoretical**, and **Cheeseburger "
         "alone was $2,819 over** (actual $3,647 vs theoretical $828). Spot-check the **3.5 oz patty "
         "roll**, cook to the board instead of over-cooking into waste, scoop fries and toppings to "
         "spec, and **log every waste item** — unlogged waste is what becomes a variance nobody can "
         "explain. Suggestive selling stays non-negotiable underneath both: milkshake, drink, fries "
         "every ticket, Halo Effect on every guest — we're **#1 in Incidence Sales company-wide** "
         "(Blackberry LTO thru Aug 16, Large soda 30¢ up). Standing checks that keep biting us: CO2 "
         "level daily (order under 25%, confirm the 50 lb backup tank), **close CT tasks the day "
         "they land** (L-Rev stacked four deep into 7/20), and the **Closing Checklist signed before "
         "the closing manager leaves** — it was missed four times in five days. People note from "
         "Brad: never run short-staffed, keep a minimum of six Shift Leaders, develop your future "
         "GMs. And the **phone scam is still live** — no email or login information over the phone, "
         "to anyone, ever."),
    ]
