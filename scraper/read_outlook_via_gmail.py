...
def build_shift_huddle_plan(today: date, categorized: dict[str, list[dict]] | None = None) -> str:
    ...
    rotating_ideas = {
        0: ("Monday — New Month, New DM, Clean Slate: Clear the Books and Get Delivery Answered",
            "First Monday of August and the board is fresh — **zero shops on the August page**, so "
            "whatever the first one is, that's the tone we set for the month. Open on the reset: "
            "**clear the CrunchTime stack** — as of Saturday it was eight deep (**L-Rev 7/29, 7/30, "
            "7/31**, **Inv-Final for Post 7/31**, **Post Period — Labor Not Posted 7/31**, "
            "**VO-Rec-BOSS Alpha–Ohio Valley** 7/29 + 7/31, **VO-Rec-Performance Fdsvce 7/31**). Post "
            "the labor and the period, receive the vendor orders, close it this morning — Brad has "
            "called out stacking repeatedly. Then **both deposits get entered**: the **7/17 book cash "
            "$433.90** has been open since 7/18, and **7/29 is $854.71 O/S**. Two weeks is too long "
            "for a drawer to sit unbooked. Time-critical this week — the **Dog Days of Summer promo "
            "runs Aug 4–7 and it is DELIVERY-ONLY** ($25+ delivery order through the app = free hot "
            "dog), and **our delivery is still OFF**. Get an answer from Brad and Jeff TODAY on "
            "whether we're re-activating before tomorrow, or we sit dark through a national promo. "
            "New leadership on the org chart: **Jeff Reiss is our District Manager** as of 7/27 "
            "(2079, 2065, 1731, 1704) — his prior store led the country on Shops and Steritech, so "
            "the bar just went up. He already filed our maintenance hub ticket (cracked tile + grill "
            "wheels) — get the **milkshake-cooler ID-tag photo** back to ticket **9780** so the right "
            "gasket gets ordered. Standing: CO2 under 25% → order now, confirm the 50 lb backup. "
            "Scams are still live — **no email, login, cash, or shipping address over the phone, "
            "ever.**"),
        1: ("Tuesday — Dog Days Starts Today, and Every Bag Still Gets Checked",
            "**The Dog Days of Summer promo goes live today and runs through Friday (Aug 4–7)** — "
            "spend **$25+ on a DELIVERY order** through the Five Guys app or website and the guest "
            "gets a free hot dog. It is **delivery-only**, and **our delivery has been OFF** pending a "
            "clean internet day. Two jobs: (1) know our status before doors — if delivery is back on, "
            "we're about to take a wave of app orders we haven't handled in weeks; if it's still off, "
            "the shift lead needs a clean answer for any guest who asks why they can't order. (2) If "
            "the orders come, **every delivery bag gets checked against the ticket item-by-item "
            "before it's sealed.** That is not a suggestion — the last complaint cluster was exactly "
            "this (7/24 a short Cajun fry, 7/24 a 30-minute wait with a wrong handoff, 7/25 a "
            "delivery missing an item, Case #5NXYSX) and Brad's directive was one line: ***\"Review "
            "order delivery accuracy validation with the team!\"*** **We have gone a full week with no "
            "new CSPs — that streak is the win, protect it.** Complaints, shops and ticket times are "
            "**60% of the KPI ranking**, so a clean week on complaints moves the whole score. Call "
            "the order back by name at handoff, catch an unhappy guest at the counter before it "
            "becomes a case, and hold tickets **under 6 minutes** so nobody waits. Sell while we're "
            "at it — Blackberry shake, drink, fries every ticket (**LTO ends Aug 16 — 12 days**), "
            "Large soda 30¢ up. We're **#1 in Incidence Sales company-wide**; we hold it by asking."),
        2: ("Wednesday — Food Cost Is Half the 55%: Portion It, Cook to the Board, Log the Waste",
            "The food side is where the money leaks, and the number is live: **COGS flash for W/E Aug "
            "2 came in at CP% 14.7 / FP% 25.2 on $3,395 CGS.** The last fully closed week landed "
            "**FP 23.0% + Labor 20.2% = 43.2% combined** — comfortably under Brad's **55% to Stay "
            "Alive** line, which ties to the bonus. FP drifting from 23.0 to 25.2 is the direction we "
            "do NOT want, so tighten it today. The last item-level breakdown we have (week of "
            "7/6–7/12) ran **$7,001 over theoretical** with **Cheeseburger alone $2,819 over** — "
            "actual $3,647 against a theoretical $828. That is patty count and grill waste, not "
            "rounding. Today: spot-check the **3.5 oz patty roll** at the grill — weigh a few, correct "
            "anybody rolling heavy; **cook to the board, not to a guess**, so we're not dumping "
            "cooked meat at the end of a rush; **every waste item gets logged** — an unlogged "
            "throw-away is the variance nobody can explain. Same discipline down the line: fries "
            "scooped to spec and not heaped, toppings measured, lids on. Sales are carrying us "
            "(**W/E Aug 2 at $23,174 vs $21,295 LY, +8.8%**) but a strong top line only pays if the "
            "food cost holds underneath it. New from the Patty Press — **review the updated Coke "
            "cartridge guidance** at the Freestyle station, and check **Blackberry runout dates** so "
            "the shake station knows when mix comes off. Tickets stay under 6 minutes while the grill "
            "runs tight; meat on before the order when 3+ are in line."),
        3: ("Thursday — Steritech Is Working Our District. Two Stores in Five Days. We're Next.",
            "This is not a drill and it is not hypothetical: **the Steritech inspector hit 1959 "
            "Lafayette on 7/27 and 1954 on 7/31** — two of our district's stores inside five days, "
            "and Brad's response both times was *\"Let's Gooo! Always Be Ready!\"* Statistically we "
            "are in the window. Run today like the inspector is already in the parking lot. Every "
            "shift: wash and sanitize ALL produce, **test the veggie wash and log it**, sanitize "
            "utensils, surfaces, dining room and restrooms, wipe the Freestyle, hand-wash and change "
            "gloves between tasks, **change sanitizer buckets** on schedule. Then the **manager walk "
            "before doors open, signed off**: hand sinks for handwashing ONLY, DayMark discard dates "
            "current, potato-cutter teeth and slicer blades chip-free (that was a CRITICAL at 1704), "
            "lids on mushrooms and onions, wet dishes stacked. **Time-and-temps get logged ON TIME, "
            "not backfilled** — that is a named accountability item and it is the single thing most "
            "likely to be missing when someone walks in. Restroom stays our systemic shop miss (two "
            "shops at -20 for dirty floor + dirty sink): named owner, hourly check, signed sheet, a "
            "real walk-in during the dinner rush. August heat is on — log refrigeration temps and "
            "watch the coolers. Our new DM came from the store that led the country on Steritech; "
            "assume he knows exactly what a ready store looks like."),
        4: ("Friday — Shop Review: We Scored a 100. August Is Blank. Go Get the First One.",
            "Pull the shop board and read it straight, because the story changed. **7/23 Dinner came "
            "back a 100%** — Bobby Cline, Francisco Llorente Mejias, Jeremiah Matthews, Madison "
            "Cureton, Richard Gibbs. That is the whole proof: this crew can run a perfect visit. July "
            "closed at **73.4% over 5 shops** and the store sits at **83.9% YTD over 42** — the July "
            "number is dragged almost entirely by the **7/11 zero**, not by how we actually run. "
            "**August is a blank page — zero shops logged.** Whatever lands first sets the month, so "
            "the ask is simple: run every shift like the shopper is in it. The points live in the "
            "**HALO EFFECT** — warm greet at the door AND the register, order called back at handoff, "
            "fries-up, a real thank-you and goodbye — plus **tickets under 6 minutes** (fry team is "
            "the named bottleneck: over 8 min = -5, over 10 min = -20), **bag accuracy checked "
            "against the ticket**, and the **milkshake offer out loud on every order** (Blackberry "
            "LTO thru Aug 16). Remind them what this store has already done: **June was three "
            "straight 100% shops (6/21, 6/24, 6/25)** and now 7/23 makes four this summer. On the "
            "manager board, **Nathan Roberts leads YTD at 94.8%**, Vicki Lucey 86.5%, Madison Cureton "
            "82.9% — and **Kasey Wilson sits at 75.1%, the only one under 80%.** Get Kasey on a shop "
            "and coach the basics. Set the weekend crew up to win one."),
        5: ("Saturday — Weekend Volume on a Thinner Bench: Cross-Cover, Sell Hard, Hold the Labor Win",
            "Saturday volume — full prep, par up bread and potatoes early. **Heads up on staffing: "
            "four terminations processed on 7/29** (Jeremiah Matthews, Angela Aaby, Richard Gibbs, "
            "Megan), so the bench is thinner than it was two weeks ago. That means **cross-cover on "
            "purpose** — everybody knows their backup station before the rush, not during it — and "
            "nobody gets stranded on fries alone. It does **not** mean we stop cutting: labor is our "
            "best story right now and we defend it. Recent days ran **7/28 21.98%, 7/29 18.78%, 7/31 "
            "18.21%** — all well under Brad's 25% ceiling, and **Friday 7/31 did it on $5,723 in net "
            "sales**, so the volume is not the excuse. Hold it the way we earned it: **check the "
            "number hourly**, trim openers and mids on a real lull, **never the close**, 30-minute "
            "breaks actually taken, employee meals rung in. Sell into the traffic — Brad's line is "
            "blunt: **suggestive selling is NOT optional.** Blackberry shake + drink + fries on every "
            "burger (**LTO ends Aug 16**), Large soda 30¢ up, every guest leaves with a 'Wow.' Keep "
            "tickets **under 6 minutes** with your fastest fry person stationed at peak and "
            "pre-dropping into the rush, and **check every bag against the ticket before it's "
            "sealed.** Steritech-clean all day — the inspector is in our district this month. Count "
            "the drawer and **enter the deposit tonight**; two are already sitting open."),
        6: ("Sunday — 7/31 Proved a Clean Close Is Possible. Make It the Standard, Not the Exception.",
            "Straight talk and then a win. The back half of last week went **7/28 — Closing Checklist "
            "MISSED, PM Pre-Shift MISSED, 9PM temp at 94% (Christopher Conder); 7/29 — Closing "
            "Checklist and Milkshake Pump Cleaning MISSED (Rusul Kadhim), 11AM temp missed (Madison "
            "Cureton).** Then **Friday 7/31 came back completely clean — every Time & Temp and the "
            "Closing Checklist done and signed.** So we know it's doable; it's a discipline gap, not "
            "a capability gap. Make 7/31 the standard: **the closing manager does not leave until the "
            "Closing Checklist and every time-and-temp are complete and signed** — on time, not "
            "backfilled the next morning. If something genuinely can't be completed, it gets written "
            "down and handed to the opener, never skipped silently. Then the rest of the close: re-par "
            "what the weekend drained (bread, potatoes, condiments, cups), DayMark labels current, "
            "drawer counted and the **deposit ENTERED** (7/17 at $433.90 and 7/29 at $854.71 are both "
            "still open), waste sheet honest, **CT tasks closed the day they land.** Recap the week "
            "for the team: sales **+8.8% over last year**, labor holding in the **18–22%** range, "
            "COGS+Labor comfortably under the 55% line, a **100% shop on 7/23**, and a full week with "
            "**no new customer complaints.** That's a good month opening — full floor discipline right "
            "through the last hour so it keeps going. Hand the openers a Monday-ready store."),
    }
    biweekly_themes = [
        ("Even Weeks — Audit-Ready Every Shift: Steritech Is In the District and Jeff Is Watching",
         "Two things changed at once and they point the same direction. **(1) Steritech is actively "
         "working our district** — the inspector hit **1959 Lafayette on 7/27** and **1954 on 7/31**, "
         "two stores in five days, with Brad calling *\"Let's Gooo! Always Be Ready!\"* both times. "
         "**(2) We have a new District Manager — Jeff Reiss, effective 7/27**, over 2079, **2065**, "
         "1731 and 1704, replacing Crystal Hess. Jeff's prior store **led the country on Shops and "
         "Steritech.** He is going to walk in with a standard already in his head, and the inspector "
         "may beat him to it. So the theme is one word: **ready.** Run it every shift: **manager "
         "walk-through completed and SIGNED before doors open** — hand sinks for handwashing only, "
         "DayMark discard dates current, potato-cutter teeth and slicer blades chip-free (a CRITICAL "
         "at 1704), lids on mushrooms and onions, wet dishes stacked, sanitizer buckets changed on "
         "schedule, veggie wash tested and logged. **Time-and-temps on time, never backfilled** — "
         "last week they slipped twice (7/28 Christopher Conder, 7/29 Rusul Kadhim) before 7/31 came "
         "back perfectly clean; 7/31 is the standard. **Restroom checked hourly with a signed sheet** "
         "— it is our systemic shop miss, two shops at -20 for dirty floor and dirty sink, and it's "
         "the first thing a guest and an inspector both notice. Ride the momentum while we do it: "
         "**7/23 Dinner scored 100%** and **August's shop board is blank** — the first one sets the "
         "month. HALO EFFECT on every guest, tickets **under 6 minutes**, **every bag checked against "
         "the ticket**, the **milkshake offered out loud** every order (Blackberry LTO thru Aug 16, "
         "Large soda 30¢ up, **#1 in Incidence Sales company-wide**). One live operational item all "
         "week: **Dog Days of Summer runs Aug 4–7 and it is delivery-only** — our delivery is OFF, so "
         "confirm status with Brad and Jeff. And the **phone scams are still live** — no email, "
         "login, cash, or shipping address over the phone, to anyone, ever."),
        ("Odd Weeks — Win the 55%: Defend the Labor Win, Kill the Food-Cost Leak",
         "One directive frames the week — Brad's **\"55% to Stay Alive\" (7/22)**: COGS + Labor "
         "combined must land **under 55%**, and it ties to the bonus. The last fully closed week came "
         "in at **43.2% combined** (FP 23.0% + Labor 20.2%) — under by almost 12 points. Two numbers "
         "hold it, pulling opposite directions. **(1) LABOR — we won it, now defend it.** Brad has "
         "pushed under 25% since 7/9 and we are living in the **18–22%** range: **7/28 21.98%, 7/29 "
         "18.78%, 7/31 18.21%** — and 7/31 did it on **$5,723 net sales**, so volume is not an "
         "excuse. Tell the team so, they earned it, then hold it the way it was earned: check the "
         "number hourly, trim openers and mids on a real lull, **never the close**, 30-minute breaks "
         "actually taken, employee meals rung in. Sales are up **+8.8% over last year** ($23,174 vs "
         "$21,295, W/E Aug 2), so hours are affordable where the volume is — the discipline lives in "
         "the slow patches. Watch coverage: **four terminations processed 7/29** thinned the bench, "
         "so cross-cover on purpose instead of overstaffing to feel safe. **(2) FOOD COST — the other "
         "half.** The flash for W/E Aug 2 was **CP% 14.7 / FP% 25.2** on $3,395 CGS — FP drifting up "
         "from 23.0% the week before, which is the wrong direction. The last item-level read (week of "
         "7/6–7/12) was **$7,001 over theoretical** with **Cheeseburger alone $2,819 over**. Spot-"
         "check the **3.5 oz patty roll**, cook to the board instead of over-cooking into waste, "
         "scoop fries and toppings to spec, and **log every waste item.** Suggestive selling stays "
         "non-negotiable underneath both — milkshake, drink, fries every ticket, Halo Effect on every "
         "guest, **#1 in Incidence Sales company-wide** (Blackberry LTO thru Aug 16, Large soda 30¢ "
         "up). Standing checks that keep biting us: **CO2 daily** (order under 25%, confirm the 50 lb "
         "backup tank), **close CT tasks the day they land** (the stack hit eight open items on 8/1), "
         "**both deposits entered** (7/17 $433.90 open since 7/18, 7/29 $854.71), the **Closing "
         "Checklist signed before the closing manager leaves**, and the **milkshake-cooler ID-tag "
         "photo** back to hub ticket 9780. Open from Brad: **Incident Process PDRs** for the entire "
         "Shift Lead team. Scams still live — nothing over the phone, ever."),
    ]
